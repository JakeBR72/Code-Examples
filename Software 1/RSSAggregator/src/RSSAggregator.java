...
/**
 * Program to convert an XML RSS (version 2.0) feed from a list of URLs into the
...
public final class RSSAggregator {
        ...
        if (channelTree.isTag()) {
            ...
            if (getChildElement(channelTree, "link") != -1) {
                ...
                html.print(channelTree
                        .child(getChildElement(channelTree, "link")).child(0)
                        .label());
                html.print("\">");
            }
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        if (parseItem.isTag()) {
            if (getChildElement(parseItem, "pubDate") >= 0) {
                html.print(parseItem
                        .child(getChildElement(parseItem, "pubDate")).child(0)
                        .label());
            } else {
        ...
        if (parseItem.isTag()) {
            ...
            if (getChildElement(parseItem, "source") >= 0) {
                html.print("<a href=\"");
                html.print(parseItem
                        .child(getChildElement(parseItem, "source"))
                        .attributeValue("url"));
                html.print("\"/>");
                html.print(parseItem
                        .child(getChildElement(parseItem, "source")).child(0)
                        .label());
                html.print("</a>");
            } else {
        ...
        if (parseItem.isTag()) {
            ...
            if (getChildElement(parseItem, "title") >= 0) {
                ...
                html.print(parseItem
                        .child(getChildElement(parseItem, "description"))
                        .child(0).label());
            } else {
    ...
    public static void main(String[] args) {
        ...
        out.println("please enter a valid XML feed:");
        XMLTree tree = new XMLTree1(in.nextLine());
        out.println("please enter an output html file:");
        SimpleWriter index = new SimpleWriter1L(in.nextLine());
        index.println("<html>\n<head>\n<title>");
        index.println(tree.attributeValue("title"));
        index.println("</title>\n</head>\n<body>\n<h1>");
        index.println(tree.attributeValue("title"));
        index.println("</h1>\n<ul>");
        int count = 0;
        while (count < tree.numberOfChildren()) {
            XMLTree child = tree.child(count);
            if (child.hasAttribute("url") && child.hasAttribute("name")
                    && child.hasAttribute("file")) {

                processFeed(child.attributeValue("url"),
                        child.attributeValue("file"), out);

                index.print("<li><a href=\"" + child.attributeValue("file")
                        + "\">");
                index.println(child.attributeValue("name") + "</a></li>");
            } else {
                out.println("<li>Bad RSS Feed Given</li>");
            }
            count++;
        }
        index.println("</ul>\n</body>\n</html>");
