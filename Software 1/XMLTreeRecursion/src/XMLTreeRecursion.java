...
public final class XMLTreeRecursion {
    ...
    private static int tagCount(XMLTree xml, String tag) {
        ...
        int numberOfTags = 0;
        int child = 0;
        while (child < xml.numberOfChildren()) {
            if (xml.child(child).label().equals(tag)) {
                numberOfTags++;
            }
            if (xml.child(child).isTag()) {
                numberOfTags += tagCount(xml.child(child), tag);
            }
            child++;
        }
        return numberOfTags;
    ...
    private static void outputTextNodes(XMLTree xml, SimpleWriter out) {
        ...
        while (child < xml.numberOfChildren()) {
            if (xml.child(child).isTag()) {
                outputTextNodes(xml.child(child), out);
            } else {
                out.println(xml.child(child).toString());
            }
            child++;
        }
