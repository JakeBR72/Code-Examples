...
public final class CryptoUtilities {
    ...
    /**
     * Useful constant, not a magic number: 2.
     */
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (!p.isZero()) {
            if (isEven(p)) {
                NaturalNumber pDivTwo = new NaturalNumber2();
                pDivTwo.copyFrom(p);
                pDivTwo.divide(TWO);
                powerMod(n, pDivTwo, m);
                ...
                NaturalNumber prePowerModN = new NaturalNumber2();
                prePowerModN.copyFrom(n);
                powerMod(n, pDivTwo, m);
                ...
                NaturalNumber prePowerModNModM = prePowerModN.divide(m);

                prePowerModNModM.multiply(n);

                n.copyFrom(prePowerModNModM.divide(m));
            }
        } else {
            n.setFromInt(1);
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        return (!(w.compareTo(ONE) == 0) || wCopy.compareTo(ONE) == 0);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        final int guessLimit = 50;
        int guessCount = 1;
        NaturalNumber guess = new NaturalNumber2();
        while (guessCount < guessLimit
                && n.compareTo(new NaturalNumber2(3)) > 0) {
            guess.copyFrom(randomNumber(nMinusOne));
            while (guess.compareTo(ONE) <= 0 || guess.compareTo(nMinusOne) >= 0) {
                guess.copyFrom(randomNumber(n));
                //System.out.println("stuck" + guess + "[]" + guessCount);
            }
