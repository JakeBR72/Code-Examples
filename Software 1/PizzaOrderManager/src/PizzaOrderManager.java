...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            item = line.substring(0, delimiter);
            price = Integer
                    .valueOf(line.substring(delimiter + 1, line.length()));
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (!done) {
            String nextTopping = input.nextLine();
            if (nextTopping.equals("")) {
                done = true;
            } else if (toppingPriceMap.hasKey(nextTopping)) {
                total += toppingPriceMap.value(nextTopping);
            }
        }

        total += sizePriceMap.value(pizzaSize);
        return total;
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        int cents = price % 100;
        if (cents < 10) {
            output.println("$" + dollars + ".0" + cents);
        } else {
