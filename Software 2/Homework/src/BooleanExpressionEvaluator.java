...
public final class BooleanExpressionEvaluator {
    ...
    public static boolean valueOfBoolExpr(Queue<String> tokens) {
        ...
        boolean valueOfBoolExpr = false;
        String token = tokens.dequeue();
        if (token == "NOT") {
            tokens.dequeue();
            valueOfBoolExpr = !valueOfBoolExpr(tokens);
            tokens.dequeue();
        } else if (token == "(") {
            tokens.dequeue();
            valueOfBoolExpr = valueOfBoolExpr(tokens);
            ...
            if (binaryOp == "AND") {
                valueOfBoolExpr = (valueOfBoolExpr && valueOfBoolExpr(tokens));
            } else {
                valueOfBoolExpr = (valueOfBoolExpr || valueOfBoolExpr(tokens));
            }
            tokens.dequeue();
        } else if (token == "T") {
            valueOfBoolExpr = true;
        }
        return valueOfBoolExpr;
    }
