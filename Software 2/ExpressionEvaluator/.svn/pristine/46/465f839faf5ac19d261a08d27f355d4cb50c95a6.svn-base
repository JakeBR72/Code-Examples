...
public final class ExpressionEvaluator {
    ...
    private static int valueOfDigit(StringBuilder source) {
        ...
        int valueOfDigit = Character.digit(source.charAt(0), RADIX);
    ...
    private static int valueOfDigitSeq(StringBuilder source) {
        assert source != null : "Violation of: source is not null";
        int valueOfDigitSeq = valueOfDigit(source);
        while (Character.isDigit(source.charAt(0))) {

            valueOfDigitSeq = (valueOfDigitSeq * RADIX) + valueOfDigit(source);
        }

        return valueOfDigitSeq;
    }
    ...
    private static int valueOfFactor(StringBuilder source) {
        ...
        if (source.charAt(0) == '(') {
            source.deleteCharAt(0);
            valueOfFactor = valueOfExpr(source);
            source.deleteCharAt(0);
        } else {
            valueOfFactor = valueOfDigitSeq(source);
        }
        return valueOfFactor;
    }
    ...
    private static int valueOfTerm(StringBuilder source) {
        assert source != null : "Violation of: source is not null";
        int valueOfTerm = valueOfFactor(source);
        while (source.charAt(0) == '*' || source.charAt(0) == '/'
                || source.charAt(0) == '%') {
            char operator = source.charAt(0);
            source.deleteCharAt(0);
            if (operator == '*') {
                valueOfTerm *= valueOfFactor(source);
            } else if (operator == '%') {
                valueOfTerm %= valueOfFactor(source);
            } else if (operator == '/') {
                valueOfTerm /= valueOfFactor(source);
    ...
    public static int valueOfExpr(StringBuilder source) {
        assert source != null : "Violation of: source is not null";
        int valueOfExpr = valueOfTerm(source);
        while (source.charAt(0) == '+' || source.charAt(0) == '-') {
            char operator = source.charAt(0);
            source.deleteCharAt(0);
            if (operator == '+') {
                valueOfExpr += valueOfTerm(source);
            } else if (operator == '-') {
                valueOfExpr -= valueOfTerm(source);
            }
        }
