...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final Pair<K, V> remove(K key) {
        ...
        Pair<K, V> remove = this.hashTable
    ...
    public final boolean hasKey(K key) {
        ...
        int count = 0;
        while (count < this.hashTable.length() && !hasKey) {
            if (this.hashTable.entry(count).size() > 0) {
                hasKey = this.hashTable.entry(count).hasKey(key);
            }
            count++;
        }
        return hasKey;
