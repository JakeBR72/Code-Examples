...
public final class Statement1PrettyPrint1 extends Statement1 {
    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            ...
            case IF: {
                ...
                Condition condition = this.disassembleIf(block);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(condition) + " THEN");
                block.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(condition, block);

                break;
            }
            case IF_ELSE: {
                Statement blockLeft = this.newInstance();
                Statement blockRight = this.newInstance();
                Condition condition = this.disassembleIfElse(blockLeft,
                        blockRight);
                printSpaces(out, offset);

                out.println("IF " + toStringCondition(condition) + " THEN");
                blockLeft.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("ELSE");
                blockRight.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");

                this.assembleIfElse(condition, blockLeft, blockRight);
                break;
            }
            case WHILE: {
                Statement block = this.newInstance();
                Condition condition = this.disassembleWhile(block);
                printSpaces(out, offset);
                out.println("WHILE " + toStringCondition(condition) + " DO");
                block.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(condition, block);
                break;
            }
            case CALL: {
    ...
    public static void main(String[] args) {
        ...
        loadStatement("data/SampleStatement.bl", s1);
        ...
        loadStatement("data/SampleStatement.bl", s2);
