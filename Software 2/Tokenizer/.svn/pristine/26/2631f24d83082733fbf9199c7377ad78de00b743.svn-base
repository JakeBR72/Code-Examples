...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        String result = "";
        int i = position;
        if (!SEPARATORS.contains(Character.toString(text.charAt(i)))) {
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        Queue tokens = new Queue2();
        while (!in.atEOS()) {
            int position = 0;
            String input = in.nextLine();
            while (position < input.length()) {
                String result = nextWordOrSeparator(input, position);
                if (!SEPARATORS.contains(result)) {
