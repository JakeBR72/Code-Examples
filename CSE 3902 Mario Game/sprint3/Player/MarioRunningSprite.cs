...
namespace MarioGame
{
    public class MarioRunningSprite : IMarioSprite
    {
        ...
        private int MillisecondsPerFrame = 70;

        public MarioRunningSprite(Texture2D texture, int columns, int startFrame, int totalFrames, Boolean leftFacing, MarioStateMachine.MarioSize size)
        {
            Shade = Color.White;
            Texture = texture;
            Columns = columns;
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (TimeSinceLastFrame > MillisecondsPerFrame)
            {
                ...
                {
                    CurrentFrame--;
                    if (CurrentFrame == StartingFrame - TotalFrames)
                    {
