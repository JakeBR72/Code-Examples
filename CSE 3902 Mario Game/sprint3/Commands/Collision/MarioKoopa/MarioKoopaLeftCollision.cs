...
namespace MarioGame
{
    public class MarioKoopaLeftCollision : ICommand
    {
        Game1 Game;
        private ICollision Side;
        private IMario Mario;
        private IKoopa Koopa;
        public MarioKoopaLeftCollision(ICollision side, Game1 game)
        {
            Game = game;
            Side = side;
            ...
            else
            {
                if (Koopa.Health == KoopaStateMachine.KoopaHealth.Shelled)
                {
                    if (!Koopa.Physics.IsMovingX())
                    {
                        ...
                        Koopa.Physics.XVelocity = Koopa.Physics.XMaxVelocity;
